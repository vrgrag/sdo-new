...
class UserService:
    ...
    def create_user(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        if UserRepository.get_by_login(user_data["login"]):
        ...
        created = UserRepository.create_user(data)
        return created
    ...
    def update_user(
        ...
        if new_login and new_login != existing["login"]:
            other = UserRepository.get_by_login(new_login)
            if other and other["id"] != user_id:
                raise HTTPException(status_code=400, detail="Логин уже занят")

        new_email = update_data.get("email")
        if new_email and new_email != existing["email"]:
            other = UserRepository.get_by_email(new_email)
            if other and other["id"] != user_id:
                raise HTTPException(status_code=400, detail="Email уже занят")
